...
void OLED_Init(void)
{
	...
	/* 配置水平寻址模式（0x20,0x00）：整屏一次I2C刷新，消除8次传输开销 */
	OLED_WriteCommand(0x20);	// 设置内存寻址模式
	OLED_WriteCommand(0x00);	// 00=水平寻址模式
	OLED_WriteCommand(0x21);	// 设置列地址范围
	OLED_WriteCommand(0);		// 列起始 = 0
	OLED_WriteCommand(127);		// 列结束 = 127
	OLED_WriteCommand(0x22);	// 设置页地址范围
	OLED_WriteCommand(0);		// 页起始 = 0
	OLED_WriteCommand(7);		// 页结束 = 7
...
/**
  ...
  * 参    数：无
  * 返 回 值：无
  * 说    明：使用水平寻址模式，整屏1024字节一次I2C写入
  *           相比逐页写入（8次I2C传输），节省约4ms/帧
  */
void OLED_Update(void)
{
	/*水平寻址模式：0x21/0x22已定义全屏范围，WriteData后光标自动归位*/
